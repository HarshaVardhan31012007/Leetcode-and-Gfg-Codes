...
public:
    ...
    const int mod=1e9+7;
    int zigZagArrays(int n, int l, int r) {
        vector<long long int>dp(r+2,0),dp2(r+2,0);
        vector<long long int>dp1(r+2,0),dp3(r+2,0);
        long long ans=0;
        int sum=0;
        for(int i=l,j=r;i<=r&&j>=l;i++,j--){
            sum++;
        ...
        for(int i=n-1;i>=1;i--){
           long long int sum=0; long long sum1=0;
           for(int prev=l,prev1=r;prev<=r&&prev1>=l;prev++,prev1--){
               sum=(sum+dp1[prev+1])%mod;
               dp2[prev]=sum;
               sum1=(sum1+dp[prev1-1])%mod;
               dp3[prev1]=sum1;
           }
           if(i==1){
               ans=(ans+sum)%mod;
